...
contract WenHeadmaster {
        ...
        returns (uint256 poolId, uint256 _amountToken, uint256 _amountETH)
    {
        ...
        address pair = uniswapV2Factory.getPair(address(token), uniswapV2Router02.WETH());
        if (pair == address(0)) pair = uniswapV2Factory.createPair(address(token), uniswapV2Router02.WETH());
        poolId = uint256(uint160(pair));
        uint256 amountTokenMin = amountToken * 95 / 100;
        uint256 amountEthMin = amountEth * 95 / 100;
        (_amountToken, _amountETH,) =
            uniswapV2Router02.addLiquidityETH{ value: amountEth }(address(token), amountToken, amountTokenMin, amountEthMin, liquidityOwner, block.timestamp);
    ...
    function getAlumni(uint256 offset, uint256 limit) external view returns (WenToken[] memory) {
