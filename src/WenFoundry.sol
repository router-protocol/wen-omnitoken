...
contract WenFoundry is ReentrancyGuard {
    ...
    address public gatewayAddress_;
    bool public paused_;
    ...
    constructor(uint256 initVirtualEthReserve, address gatewayAddress) {
        feeTo_ = msg.sender;
        owner_ = msg.sender;
        paused_ = false;
    ...
    modifier onlyWenToken(WenToken token) {
        if (token == WenToken(address(0)) || pools_[token].token != token)
            revert NotWenToken();
        _;
    }
    ...
    event TokenGraduated(
        WenToken indexed token,
        WenHeadmaster indexed headmaster,
    ...
    function _deployToken(
        ...
        string memory description,
        string memory extended
    ) private returns (WenToken) {
        WenToken token = new WenToken(
            name,
            symbol,
            DECIMALS,
            TOTAL_SUPPLY,
    ...
    function _graduate(WenToken token) private {
        ...
        WenToken(address(token)).setIsUnrestricted(true);
    ...
    function _swapEthForTokens(
        ...
        WenLedger.Trade memory trade = WenLedger.Trade(
            token,
            true,
            msg.sender,
            amountIn + fee,
    ...
    function getPoolsAll(
        ...
        WenToken[] memory tokens = wenLedger_.getTokens(offset, limit);
